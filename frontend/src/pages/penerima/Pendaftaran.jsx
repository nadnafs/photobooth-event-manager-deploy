...
const Pendaftaran = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      window.open(`/print-nota/${res.data.transaction.id}`, '_blank');
      ...
      <div>
        <div className="bg-card p-6 rounded-2xl shadow-sm border border-border sticky top-6">
          ...
          <div className="border-t border-border mt-6 pt-4 space-y-4">
            ...
            <button type="submit" form="pendaftaran-form" className="w-full py-3.5 bg-primary text-white rounded-xl font-bold text-lg hover:bg-blue-700 transition-colors shadow-sm mt-4 flex justify-center items-center gap-2">
              <Check size={20} /> Simpan Pendaftaran
            </button>
          </div>
        </div>
      </div>
